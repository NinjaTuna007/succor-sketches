// hybrid_tx_rx_microros_OCXO_EMAfiltering.ino
//
// Teensy 4.1
...
//   - now publishes delta_filtered topic (could remove this and just do it onboard the computer, but for Lolo we won't have access to it)
//
// micro-ROS options:
...
static char topic_delta_filtered[96];
#endif
...
static std_msgs__msg__Float32  msg_delta_filtered;
#endif
#endif
// =======================================================
...
static volatile float    dbg_last_flag_filtered_delta_us = 0.0f;
...
static constexpr float FAST_EMA_ALPHA_SLOW   = 0.02f;
static constexpr float FAST_EMA_ALPHA_FAST   = 0.55f;
static constexpr float FAST_EMA_THRESHOLD_US = 300.0f;

static bool  fast_ema_initialized = false;
static float fast_ema_delta_us    = 0.0f;

static float update_fast_adaptive_ema(int32_t raw_delta_us)
{
  float x = (float)raw_delta_us;

  if (!fast_ema_initialized) {
    fast_ema_delta_us = x;
    fast_ema_initialized = true;
    return fast_ema_delta_us;
  }

  float err = fabsf(x - fast_ema_delta_us);

  float gain = err / FAST_EMA_THRESHOLD_US;
  if (gain > 1.0f) gain = 1.0f;

  float alpha =
    FAST_EMA_ALPHA_SLOW +
    (FAST_EMA_ALPHA_FAST - FAST_EMA_ALPHA_SLOW) * gain;

  fast_ema_delta_us =
    alpha * x +
    (1.0f - alpha) * fast_ema_delta_us;
...
static void print_debug_snapshot()
{
  ...
  foat32_t last_flag_filtered_delta_us;
  ...
  last_flag_filtered_delta_us   = dbg_last_flag_filtered_delta_us;
  ...
  Serial.printf(
  "MODE=%s | LAST_SRC=%s | SIT_now=%u us | TeensyMicros=%u us | TRIG_count=%u | EPOCH=%u us | LAST_REAL_PPS=%u us | prev_TRIG=%u us | dTRIG=%ld us | FLAG_count=%u | FLAG_ts=%u us | dFLAG=%ld us | FLAG_ignored=%u | dFLAG_fast=%.3f us\n",
  mode_str,
  src_str,
  sit_now,
  teensy_now,
  ...
  ignored_flag_count,
  (double)last_flag_filtered_delta_us
  );
}
#endif
// =======================================================
...
#if !MODE_TRANSMITTER
static float update_and_print_delta_filters(int32_t raw_delta)
{
  float fast_delta = update_fast_adaptive_ema(raw_delta);

#if ENABLE_SERIAL_DEBUG
  Serial.printf(
    "DELTA raw_deltas=%ld us | fast_ema_deltas=%.3f us\n",
    (long)raw_delta,
    (double)fast_delta
  );
#endif
...
void setup()
{
  ...
  std_msgs__msg__UInt32__init(&msg_pps);
    ...
    make_topic(topic_delta_filtered, sizeof(topic_delta_filtered), "/succorfish/delta_t_filtered");
    ...
    require_ok(
      ...
      require_ok(
      rclc_publisher_init_best_effort(
        &pub_delta_filtered, &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32),
        topic_delta_filtered),
      7);

    std_msgs__msg__UInt32__init(&msg_flag);
    std_msgs__msg__Int32__init(&msg_delta);
    std_msgs__msg__Float32__init(&msg_delta_filtered);
...
void loop()
{
  ...
  if (flag_pending) {
    ...
    if (!armed_for_flag) {
      ...
      int32_t raw_delta_us = (int32_t)du;
      float filtered_delta_us = update_and_print_delta_filters(raw_delta_us); 
      
      
      dbg_last_flag_us = flag_us;
      dbg_last_flag_delta_us = (int32_t)du;
      dbg_last_flag_filtered_delta_us = filtered_delta_us;
      dbg_flag_count++;
      ...
      msg_delta_filtered.data = filtered_delta_us;
      (void)rcl_publish(&pub_delta_filtered, &msg_delta_filtered, NULL);
#endif
